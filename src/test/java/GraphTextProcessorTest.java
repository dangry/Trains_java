...
public class GraphTextProcessorTest {
    ...
    private static final String NUMBER_OF_TRIPS_MAX_STOPS_STRING = "The number of trips starting at C and ending at C with a maximum of 3 stops.";
    private static final String NUMBER_OF_TRIPS_EXACT_STOPS_STRING = "The number of trips starting at A and ending at C with exactly 4 stops.";
    private static final String SHORTEST_ROUTE_STRING = "The length of the shortest route (in terms of distance to travel) from A to C.";
    private static final String DIFFERENT_ROUTES_MAX_DISTANCE_STRING = "The number of different routes from C to C with a distance of less than 30.  In the sample data, the trips are: CDC, CEBC, CEBCDC, CDCEBC, CDEBC, CEBCEBC, CEBCEBCEBC.";
    ...
    public void shouldCheckIfDistanceInstructionIsValid() {
        assertEquals(ActionType.ROUTE_DISTANCE,
                textProcessor.getActionType(ROUTE_DISTANCE_STRING));
    }

    @Test
    public void shouldCheckIfNumberOfTripsMaxStopsInstructionIsValid() {
        assertEquals(ActionType.NUMBER_OF_TRIPS_MAX_STOPS,
    ...
    public void shouldCheckIfNumberOfTripsExactStopsInstructionIsValid() {
        assertEquals(ActionType.NUMBER_OF_TRIPS_EXACT_STOPS,
                textProcessor.getActionType(NUMBER_OF_TRIPS_EXACT_STOPS_STRING));
    }

    @Test
    public void shouldCheckIfShortestRouteInstructionIsValid() {
        assertEquals(ActionType.SHORTEST_ROUTE,
                textProcessor.getActionType(SHORTEST_ROUTE_STRING));
    }

    @Test
    public void shouldCheckIfDifferentRoutesMaxDistanceInstructionIsValid() {
        assertEquals(ActionType.DIFFERENT_ROUTES_MAX_DISTANCE,
    ...
    public void shouldCreateTrainWithTextData() {
        assertEquals('C', textProcessor.createTrainFromData("CC30",
                ActionType.DIFFERENT_ROUTES_MAX_DISTANCE).currentNodeName);
    }
